        ...
        <div class="tab-content">
          ...
          <div class="tab-pane fade" id="todayTab">
            ...
            <?php 
                ...
                $today_profiles = getTodayProfiles('tracker_profile_form', $login_id);
            ...
            <tr data-search="<?= strtolower( $row->name.' '.$row->contact.' '.$row->profile_id) ?>" >
                 <td><?php echo $x++;?></td>
                  <td>
                    <strong><span class="text-success"><?php echo $row->profile_id ?></span></strong><br>
                    <strong><span class="text-danger"><?php echo $row->name ?></span></strong><br>
                    <strong><span class="text-warning"><?php echo $row->contact ?></span></strong><br>
                  </td>
            ...
            <?php } }else{ ?>
                <tr><td colspan="3" class="text-danger text-center">No profiles Created today</td></tr>
           <?php } ?>
              </tbody>
            </table>
          </div>
          ...
          <div class="tab-pane fade show active" id="allTab">
            ...
            <!--    <input type="text" id="searchInput" class="form-control" placeholder="Search by Name / Contact / Profile ID">-->
            ...
            <table class="table custom-table">
              ...
              <tbody>
                <?php
                // $login_id = $this->session->userdata('login_id');
                // $get_data = getAllDataById('tracker_profile_form',$login_id);
                 if(!empty($get_data)){
                 $x = 1;
                 ...
                 <tr data-search="<?= strtolower( $row->name.' '.$row->contact.' '.$row->profile_id) ?>" >
                  <td><?php echo $x++;?></td>
                  <td>
                    <stron><span class="text-succes"><?php echo $row->profile_id ?></span></strong><br>
                    <stron><span class="text-dange"><?php echo $row->name ?></span></strong><br>
                    <stron><span class="text-warnin"><?php echo $row->contact ?></span></strong><br>
                  </td>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <p><strong>Profile Type:</strong> <span id="m_profileType" class="text-uppercase"></span></p>  
...
function openDetail(profile_id, profile_type, name,mobile,address,type){
  ...
  document.getElementById('m_profileType').innerText = profile_type;
...
function filterByDate() {
    ...
    if (from && to) {
        window.location.href =
          "<?= base_url('profile-created') ?>?from=" + from + "&to=" + to;
    }
}
