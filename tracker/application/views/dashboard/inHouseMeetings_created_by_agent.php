        ...
        <div class="tab-content">
          ...
          <div class="tab-pane fade" id="todayTab">
            ...
            <?php 
                ...
                $today_profiles = getTodayProfiles('tracker_in_house_working', $login_id);
            ...
            ?>
             <tr data-search="<?= strtolower( $row->name.' '.$row->contact.' '.$row->profile_id) ?>" >
                 <td><?php echo $x++;?></td>
                   <td>
                    <span><strong>Meeting With:</strong> <?= ucfirst($row->meeting ?? '-') ?></span><br>
                    <span><strong>Name:</strong> <?=ucfirst($row->name) ?? '-' ?></span><br>
                    <span><strong>Room No:</strong> <?= $row->room_no ?? '-' ?></span><br>
                   </td>
                  ...
                  <td class="text-center">
                    ...
                    <i class="bi bi-eye action-icon text-success"
                       ...
                       onclick="openDetail(
                            ...
                            '<?= !empty($row->from_time) ? date('h:i A', strtotime($row->from_time)) : '-' ?>',
                            '<?= !empty($row->to_time) ? date('h:i A', strtotime($row->to_time)) : '-' ?>'
                        )">
                    </i> 
                  </td>
                </tr>
            <?php } }else{ ?>
                <tr><td colspan="3" class="text-danger text-center">No In House Meetings Created today.</td></tr>
           <?php } ?>
              </tbody>
            </table>
          </div>
          ...
          <div class="tab-pane fade show active" id="allTab">
            ...
            <table class="table custom-table">
              ...
              <tbody>
                <?php
                $login_id = $this->session->userdata('login_id');
                // $get_data = getAllDataById('tracker_in_house_working',$login_id);
            ...
            <?php } }else{ ?>
               <tr><td colspan="3" class="text-danger text-center">No In House Meetings Created.</td></tr>
            <?php } ?>
              </tbody>
            </table>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-header">
        <h5 class="modal-title">In House Meetings Details</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>

     <div class="modal-body">
        <p><strong>Name:</strong> <span id="m_name"></span></p>
        <p><strong>Meeting:</strong> <span id="m_meeting"></span></p>
        <p><strong>Date:</strong> <span id="m_date"></span></p>
        <p><strong>From Time:</strong> <span id="m_from"></span></p>
        <p><strong>To Time:</strong> <span id="m_to"></span></p>
    </div>
...
<script>
function openDetail(name, meeting, date, from, to){
  document.getElementById('m_name').innerText = name;
  document.getElementById('m_meeting').innerText = meeting;
  document.getElementById('m_date').innerText = date;
  document.getElementById('m_from').innerText = from;
  document.getElementById('m_to').innerText = to;
}
...
function filterByDate() {
    ...
    if (from && to) {
        window.location.href =
          "<?= base_url('inhousemeetings-created') ?>?from=" + from + "&to=" + to;  
    }
}
